...
func (t *Table2Struct) Run() error {
	...
	// 组装struct
	if t.config.SeparateFile {
		return t.writeSeparateFile(tableColumns, packageName)
	} else {
		return t.writeSingleFile(tableColumns, packageName)
	}
}

func (t *Table2Struct) writeSeparateFile(tableColumns map[string][]column, packageName string) error {
	structContentes := make(map[string]string)
	for tableRealName, item := range tableColumns {
		structContent := t.makeStructContent(tableRealName, item)
		// 如果有引入 time.Time, 则需要引入 time 包
		var importContent string
		if strings.Contains(structContent, "time.Time") {
			importContent = "import \"time\"\n\n"
		}
		structContentes[tableRealName] = packageName + importContent + structContent
	}
	...
	for fileName, fileContent := range structContentes {
		filePath := fmt.Sprintf("%s/%s.go", savePath, fileName)
...
func (t *Table2Struct) writeSingleFile(tableColumns map[string][]column, packageName string) error {
	...
	fileContent := packageName + importContent + structContent
	return writeFile(filePath, fileContent)
}

func (t *Table2Struct) makeStructContent(tableRealName string, columns []column) string {
	var structContent string
	// 去除前缀
	if t.prefix != "" {
